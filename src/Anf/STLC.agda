...
Env : ∀{n} → Ctx n → Set
Env {n} _ = Vec AnfVal n

private variable
  fresh fresh' fresh'' : ℕ

data _⊢⟨_,_⟩⇝_⟨_,_⟩ : ∀{Γ : Ctx n} →
    Env Γ → Tm τ Γ → ℕ → List AnfTm → AnfVal → ℕ → Set
  where
  anf-C : ∀{Γ : Ctx n} {env : Env Γ} {x} →
    env ⊢⟨ C {Γ = Γ} x , fresh ⟩⇝ []ₗ ⟨ Constₐ x , fresh ⟩
  anf-V : ∀{Γ : Ctx n} {env : Env Γ} {i e} →
    {rlookup env i ≡ e} →
    env ⊢⟨ V {Γ = Γ} i {refl} , fresh ⟩⇝ []ₗ ⟨ e , fresh ⟩
  anf-λ : ∀{Γ : Ctx n} {env : Env Γ} {tms} {e : Tm τ' (τ ∷ Γ)} {body} →
    (Varₐ fresh ∷ env) ⊢⟨ e , suc fresh ⟩⇝ tms ⟨ body , fresh' ⟩ →
    env ⊢⟨ λ' τ e , fresh ⟩⇝ [ λₐ τ tms body ] ⟨ Varₐ fresh , suc fresh ⟩
  anf-$ : ∀{Γ : Ctx n} {env : Env Γ} {e₁ : Tm (τ₁ => τ₂) Γ} {e₂ v₁ v₂ tms₁ tms₂} →
    env ⊢⟨ e₁ , fresh ⟩⇝ tms₁ ⟨ v₁ , fresh' ⟩ →
    env ⊢⟨ e₂ , fresh' ⟩⇝ tms₂ ⟨ v₂ , fresh'' ⟩ →
    env ⊢⟨ e₁ $ e₂ , fresh ⟩⇝ ((v₁ $ₐ v₂) ∷ₗ tms₁ ++ tms₂) ⟨ Varₐ fresh'' , suc fresh'' ⟩
  anf-+ : ∀{Γ : Ctx n} {env : Env Γ} {e₁ : Tm _ Γ} {e₂ v₁ v₂ tms₁ tms₂} →
    env ⊢⟨ e₁ , fresh ⟩⇝ tms₁ ⟨ v₁ , fresh' ⟩ →
    env ⊢⟨ e₂ , fresh' ⟩⇝ tms₂ ⟨ v₂ , fresh'' ⟩ →
    env ⊢⟨ e₁ + e₂ , fresh ⟩⇝ ((v₁ +ₐ v₂) ∷ₗ tms₁ ++ tms₂) ⟨ Varₐ fresh'' , suc fresh'' ⟩
  anf-let : ∀{Γ : Ctx n} {env : Env Γ} {e₁} {e₂ : Tm τ₂ (τ₁ ∷ Γ)} {v₁ v₂ tms₁ tms₂} →
    env ⊢⟨ e₁ , fresh ⟩⇝ tms₁ ⟨ v₁ , fresh' ⟩ →
